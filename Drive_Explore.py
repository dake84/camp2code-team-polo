...
def vary(value: int, amount: int, low: float = 0.95, high: float = 1.05) -> int:
    """_summary_

    Args:
        value (int): _description_
        amount (int): _description_
        low (float, optional): _description_. Defaults to 0.95.
        high (float, optional): _description_. Defaults to 1.05.

    Returns:
        int: _description_
    """
    return int(value + amount * random.uniform(low, high))
...
def clamp(value: int, min_v: int, max_v: int) -> int:
    ...
    Args:
        value (int): _description_
        min_v (int): _description_
        max_v (int): _description_
...
def drive_explore(self, actual_speed: int, steering_angle: int):
    ...
    """    
    speed_var = vary(actual_speed, 5)
    steer_var = vary(steering_angle, 5)

    actual_speed = clamp(speed_var, 30, 100)
    steering_angle = clamp(steer_var, 45, 135)
